...
初回配信コードは `df3cc4b567b3`、Cloudflare Version IDは `6cfc7577-f13b-4511-ac33-41854544689f`。更新前後で本番デプロイ履歴と公開HTML／JS／CSSのSHA-256が一致した。最新の配信情報は [staging-deployment-2026-09-16.md](staging-deployment-2026-09-16.md) を参照。
...
1. シーク直後に発音補正を+500msから0msへ変えると、未送信イベントがシーク位置より前になり、対応する音声ブロックがないためMIDI全体が待ち続けていた。保持している最古の位置より前に来たイベントを順番どおり処理するよう修正。最初の音声ブロックが出力待ちならその時刻まで待ち、既に再生済みなら現在時刻で適用する。未来のイベントは従来どおり対応するブロックを待つ。
2. SoundFontの交換判定がシーク準備中を停止中と見なしていた。MDRの準備開始から音声接続まで世代番号で保護し、復元バッチ間も交換を拒否するよう修正。交換処理の開始時と反映直前の両方で確認する。正常完了・エラー・停止で保護を解除するが、旧世代の終了処理で次の準備の保護を解除しない。

新たな6件の回帰テストは修正前に失敗し、修正後に成功。全213件／18ファイル、通常・Cloudflare向け型チェック、検証用ビルド、`git diff --check` 成功。

Chrome実音源の検査にも、実際のMIDIノートの直後へシークして+500ms→0msに切り替える操作と、復元完了通知後・準備完了前のSoundFont交換試行を追加した。`song.mdr`＋`sample.pdx`と`BIN_M_GS.MDR`、GeneralUser-GSをローカルで使用し、desktop／mobileバッファ設定の計4条件すべてでMIDI発音継続、交換拒否、従来のシーク・ループ・終了の成功、ブラウザ例外0件を確認。Safari・モバイル実機の確認は未実施。
