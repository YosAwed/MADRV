...
const before = bytes('madrv-mdx-player-v15_6c603674.wasm');
const after = bytes('madrv-mdx-player-v16_865de60b.wasm');
function fixture(voice, pcm8, pan) {
  const parts = Array.from({length: pcm8 ? 16 : 9}, (_, n) => Buffer.from(n === voice + 8
    ? [0xfc, pan, 0xfb, 15, 0xed, 4, 0xf8, 8, 0x80, 47, 0x80, 47, 0x80, 47, 47, 0xf1, 0]
...
async function render(binary,voice,pcm8,pan) {
  ...
  const audio=new Int16Array(144000*2);let left=0,right=0;const pans=new Set(), triggers=new Set();
  try {
    for(let frame=0;frame<144000;frame+=256){const count=Math.min(256,144000-frame);p._mdx_player_render(out,count);const block=p.HEAP16.subarray(out/2,out/2+count*2);audio.set(block,frame*2);
      for(let i=0;i<block.length;i+=2){left+=Math.abs(block[i]);right+=Math.abs(block[i+1]);}
      if(p.asm.get_pcm_trigger_count){triggers.add(p.asm.get_pcm_trigger_count(voice));for(let other=0;other<8;other++)if(other!==voice)assert.equal(p.asm.get_pcm_trigger_count(other),0);}
    ...
    return {audio,left,right,pans:[...pans],triggers:[...triggers]};
...
for(const pcm8 of [false,true])for(let voice=0;voice<(pcm8?8:1);voice++)for(const pan of [0,1,2,3]){
  ...
  assert.deepEqual(a.audio,b.audio,'trigger telemetry must preserve every audio sample');
  assert.deepEqual(b.triggers.filter(value=>value>0),[1,2,3], 'each repeated same-number note must increment once');
...
console.log(JSON.stringify({cases,bitIdentical:true,repeatedNotes:true,mappings}));
