  ...
  return (
    <div className="ui-dense compact-deck relative min-h-screen">
      ...
      <main className="compact-main">
        <section className="compact-transport primary-deck" aria-label="再生コントロール">
          <div className="compact-title-row"><h1 data-testid="source-title" title={sourceTitle}>{sourceTitle}</h1><span className="mono compact-clock">{formatTime(elapsed)} / {duration ? formatTime(duration) : "--:--.---"}</span><span className="mono compact-tempo">{estimatedBpm ? `${estimatedBpm.toFixed(1)} BPM` : "— BPM"}</span></div>
...
<div className="mt-3 flex flex-wrap items-center gap-3"><p className="mono text-[10px] leading-5 text-[#a9aca2]">MDR／MDXとPDXは同時選択、または後から追加できます。</p><button onClick={() => folderInputRef.current?.click()} className="mono shrink-0 border border-primary/45 bg-primary/[0.045] px-3 py-2 text-[9px] uppercase tracking-[0.08em] text-primary transition-colors hover:bg-primary hover:text-primary-foreground">Select folder / playlist</button></div></div>}
              {mode === "remote" && <div className="compact-source-form"><div><div className="flex items-center justify-between gap-3"><SmallLabel>Remote MDR / MDX URL</SmallLabel>{isCloudShareLink(remoteMdr) && <span className="mono text-[9px] uppercase tracking-[0.09em] text-primary">Share proxy ready</span>}</div><input value={remoteMdr} onChange={(event) => setRemoteMdr(event.target.value)} placeholder="https://storage.example/song.mdr または song.mdx／Drive・Dropbox共有リンク" className="mono mt-2 w-full border border-white/15 bg-[#11120f] px-3 py-3 text-xs text-[#f5f4ec] outline-none placeholder:text-[#62675d] focus:border-primary" /></div>
...
<CompactPanel id="catalog" title="リモートカタログ" summary={`${catalogEntries.length} 曲`}><div className="flex items-center justify-between"><SmallLabel>Remote catalog / JSON</SmallLabel><span className={`mono text-[9px] ${catalogIsDriveLink ? "text-primary" : "text-[#a9aca2]"}`}>{catalogIsDriveLink ? "DRIVE DETECTED" : "CORS REQUIRED"}</span></div>
...
<p className="mono mt-2 text-[9px] leading-4 text-[#a9aca2]">配列または<code>entries</code> / <code>tracks</code> / <code>songs</code>配列を受け付けます。これをリモートフォルダ用マニフェストとして扱い、各項目は<code>mdrUrl</code>（MDXも可）と任意の<code>pdxUrl</code>を指定します。Google Drive／Dropboxは<strong>公開共有JSONファイル</strong>を指定してください。フォルダのHTML一覧は取得しません。</p></CompactPanel></div>}
            </CompactPanel>
            <CompactPanel id="playlist" title="プレイリスト" summary={`${playlistEntries.length} 曲`}><div className="compact-playlist" aria-label="プレイリスト" data-testid="saved-playlist">
...
<p className="mt-2 text-xs leading-5 text-[#a9aca2]">GS向けSF2/DLSをローカルから選べます。MMLの@MIDIおよびMDRのMIDIトラックに使用します。</p>
              ...
              <CompactPanel id="remote-soundfont" title="SoundFontをURLから読み込む" summary={remoteSoundfontLoading ? "Loading…" : undefined}><div className="flex items-center justify-between"><SmallLabel>Remote SoundFont / CORS</SmallLabel><span className="mono text-[8px] text-[#a9aca2]">DIRECT / RANGE FALLBACK · MAX 320 MB</span></div>
...
<p className="mono mt-2 text-[8px] leading-4 text-[#72776d]">Default SoundFontはこのボタンだけで自動ロードします。CORS対応URLはブラウザから直接読み込みます。Google Drive／Dropbox共有リンクは、直接読込が許可されない場合だけ8 MB単位の補助取得へ切り替え、最大320 MBまで読み込みます。共有設定は「リンクを知っている全員・閲覧者」、ダウンロード制限は解除してください。</p>{remoteSoundfontProgress && <div data-testid="remote-soundfont-progress" className={`mt-3 border p-2.5 ${remoteSoundfontProgress.stage === "failed" ? "border-[#ff9b94]/50 bg-[#ff9b94]/[0.04]" : "border-primary/30 bg-primary/[0.035]"}`}><div className="flex items-center justify-between gap-3"><span className={`mono text-[8px] uppercase tracking-[0.08em] ${remoteSoundfontProgress.stage === "failed" ? "text-[#ff9b94]" : "text-primary"}`}>{remoteSoundfontProgress.stage === "downloading" ? "SoundFont downloading" : remoteSoundfontProgress.stage === "initializing" ? "Download complete · initializing" : remoteSoundfontProgress.stage === "ready" ? "SoundFont ready" : "SoundFont load failed"}</span><span data-testid="remote-soundfont-progress-percent" className="mono text-[9px] text-[#dfe1d8]">{remoteSoundfontProgress.totalBytes && remoteSoundfontProgress.totalBytes > 0 ? `${Math.min(100, Math.round(remoteSoundfontProgress.loadedBytes / remoteSoundfontProgress.totalBytes * 100))}%` : "SIZE UNKNOWN"}</span></div><div role="progressbar" aria-label="SoundFont download progress" aria-valuemin={0} aria-valuemax={remoteSoundfontProgress.totalBytes ?? undefined} aria-valuenow={remoteSoundfontProgress.totalBytes ? Math.min(remoteSoundfontProgress.loadedBytes, remoteSoundfontProgress.totalBytes) : undefined} aria-valuetext={remoteSoundfontProgress.totalBytes ? `${formatByteSize(remoteSoundfontProgress.loadedBytes)} / ${formatByteSize(remoteSoundfontProgress.totalBytes)}` : `${formatByteSize(remoteSoundfontProgress.loadedBytes)} received`} className="mt-2 h-px overflow-hidden bg-white/15"><div className={`h-full transition-[width] duration-150 ${remoteSoundfontProgress.stage === "failed" ? "bg-[#ff9b94]" : "bg-primary"} ${remoteSoundfontProgress.totalBytes ? "" : "animate-pulse"}`} style={{ width: `${remoteSoundfontProgress.totalBytes && remoteSoundfontProgress.totalBytes > 0 ? Math.max(0, Math.min(100, remoteSoundfontProgress.loadedBytes / remoteSoundfontProgress.totalBytes * 100)) : 16}%` }} /></div><p data-testid="remote-soundfont-progress-bytes" className="mono mb-0 mt-2 text-[8px] leading-4 text-[#a9aca2]">{formatByteSize(remoteSoundfontProgress.loadedBytes)}{remoteSoundfontProgress.totalBytes ? ` / ${formatByteSize(remoteSoundfontProgress.totalBytes)}` : " received · total size not provided"}{remoteSoundfontProgress.stage === "initializing" ? " · SoundFontを初期化中" : remoteSoundfontProgress.stage === "ready" ? " · ready" : ""}</p></div>}</CompactPanel>
            ...
            <CompactPanel id="share" title="セッション共有" summary="Short link"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><SmallLabel>Share session / short link</SmallLabel><p className="mt-1 text-xs leading-5 text-[#a9aca2]">公開MDR／PDX／SoundFont URL、MML、ループ上限を短い共有IDで復元します。音声・ローカルファイル・外部MIDI機器・診断ログは共有しません。既存の長い共有URLも引き続き開けます。</p></div><Button onClick={copySessionLink} disabled={sharedSessionCreate.isPending} variant="outline" className="h-10 shrink-0 rounded-none border-primary/50 px-3 text-[10px] font-medium uppercase tracking-[0.08em] text-primary hover:bg-primary/10"><Share2 size={14} />{sharedSessionCreate.isPending ? "Creating" : "Copy short link"}</Button></div>
{sessionLink && <input value={sessionLink} readOnly onFocus={(event) => event.currentTarget.select()} aria-label="共有セッションリンク" className="mono mt-3 w-full border border-white/15 bg-[#11120f] px-3 py-2 text-[9px] text-[#dfe1d8] outline-none focus:border-primary" />}</CompactPanel>
          ...
          <div className="compact-column">
            ...
            <CompactPanel id="diagnostics" title="再生情報" summary={`${(audioSampleRate / 1000).toFixed(1)} kHz`}><div data-testid="playback-diagnostics" className="program-diagnostics grid gap-px border border-white/10 bg-white/10 sm:grid-cols-3">
            ...
            <CompactPanel id="sysex" title="GSパート設定 / SysEx" summary={`Part ${gsPart}`}><div className="flex items-center justify-between"><SmallLabel>GS SysEx / Function call</SmallLabel><span className="mono text-[9px] text-[#a9aca2]">DT1</span></div>
...
<p className="mt-2 text-xs leading-5 text-[#a9aca2]">GS Reset、Part Rx On/OffはRoland DT1形式で送出します。テストはC4を約0.4秒鳴らし、必ずAll Notes Offで終了します。</p></CompactPanel>
            <CompactPanel id="midi-log" title="MIDI診断ログ" summary={`${midiDiagnostics.length} 件`}><div className="flex items-center justify-between"><SmallLabel>Hardware diagnostic</SmallLabel><span className={`mono text-[9px] ${midiOutputMode === "hardware" ? "text-primary" : "text-[#a9aca2]"}`}>{midiOutputMode === "hardware" ? "OUTPUT ARMED" : "SOUNDFONT MODE"}</span></div>
...
<button onClick={exportMidiDiagnostics} disabled={!midiDiagnostics.length} className="mono mt-2 w-full border border-white/15 bg-[#11120f] px-2 py-2 text-[8px] uppercase tracking-[0.08em] text-[#dfe1d8] transition-colors hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-35">Export diagnostic JSON</button></CompactPanel>
            <CompactPanel id="export" title="動画書き出し" summary="MP4 / WebM"><div className="compact-export"><label className="block"><SmallLabel>Export maximum / sec</SmallLabel><input type="number" min="10" max="600" value={exportLimit} onChange={(event) => setExportLimit(Math.max(10, Math.min(600, Number(event.target.value) || 60)))} className="mono mt-2 w-full border border-white/15 bg-[#11120f] px-3 py-2 text-xs text-[#f5f4ec] outline-none focus:border-primary" /></label>
<Button onClick={exportMml} disabled={isExporting} className="self-end rounded-none bg-[#e4e6dc] px-4 py-5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#11120f] hover:bg-primary">{isExporting ? <LoaderCircle size={15} className="animate-spin" /> : <FileAudio size={15} />}{isExporting ? "Rendering" : "Export MP4"}</Button></div><p className="mono text-xs">∞設定でも書き出しは1回・最大秒数に制限されます。</p></CompactPanel>
          ...
          <p className="mono m-0 text-[10px] uppercase tracking-[0.17em] text-[#72776d]">MADRV Player / Signal Deck</p>
              ...
              <p className="text-xs">音源はブラウザで扱います。CORS対応URLと、公開共有されたGoogle Drive／Dropboxに対応します。</p>
            </CompactPanel>
          </div>
        </div>
        {activeMixerTracks.length > 0 && <div className="compact-tracks">
          <CompactPanel id="activity" title="トラック発音状況" summary={`${activeMixerTracks.length} tracks`}><div data-testid="track-key-overview"><ChannelNoteState tracks={activeMixerTracks} trackKeyState={trackKeyState} mutedTracks={mutedTracks} pcmActivityMask={pcmActivityMask} /></div></CompactPanel>
          <CompactPanel id="matrix" title="鍵盤 / Track matrix" summary={`${activeMixerTracks.length} tracks · ${mutedTracks.length} muted`}><div className="compact-matrix"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 pb-2">
