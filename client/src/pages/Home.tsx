...
function SmallLabel({ children }: { children: React.ReactNode }) {
  return <p className="mono m-0 text-[10px] font-medium uppercase tracking-[0.2em] text-[#a9aca2]">{children}</p>;
}
...
function MdrMetadataPanel({ info, linkedPdxName, sourceLabel, estimatedDuration }: { info: MdrInfo; linkedPdxName: string; sourceLabel: string; estimatedDuration: number | null | undefined }) {
  ...
  return <div className="mt-4 border border-primary/30 bg-primary/[0.045] p-4" aria-label="MDR metadata">
    ...
    <dl className="mt-3 grid gap-x-4 gap-y-3 sm:grid-cols-2">
      ...
      <div><dt className="mono text-[9px] uppercase tracking-[0.12em] text-[#a9aca2]">Estimated duration</dt><dd className="mono mt-1 text-xs text-[#f5f4ec]">{estimatedDuration === undefined ? "Preparing…" : estimatedDuration === null ? (requiresPdx && !linkedPdxName ? "Unavailable until PDX is linked" : "Calculated when playback starts") : formatTime(estimatedDuration)}</dd></div>
...
function PlaybackAdvisorPanel({ diagnosis, activeProfile, tuningPreset, safariCompatibilityMode, onSelect }: { diagnosis: LoadDiagnosis; activeProfile: PlaybackPerformanceProfile; tuningPreset: "auto" | PlaybackTuningPreset; safariCompatibilityMode: boolean; onSelect: (preset: "auto" | PlaybackTuningPreset) => void }) {
  ...
  return <section data-testid="playback-advisor" className="mt-4 border border-[#8fa7cc]/40 bg-[#8fa7cc]/[0.06] p-4" aria-label="端末負荷診断">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><SmallLabel>Playback advisor</SmallLabel><p className="mono mb-0 mt-2 text-xs text-[#f5f4ec]">{diagnosis.measuring ? "端末負荷を測定中…" : recommendation ? `推奨: ${presetLabels[recommendation.preset]} / 負荷 ${recommendation.score} / 100` : "音源の読み込み後に端末負荷を診断します。"}</p></div><span className="mono border border-[#8fa7cc]/45 px-2 py-1 text-[9px] uppercase tracking-[0.1em] text-[#c2d6f4]">{tuningPreset === "auto" ? `auto · ${activeLabel}` : presetLabels[tuningPreset]}</span></div>
    {recommendation && <><p className="mono mb-0 mt-3 text-[9px] leading-5 text-[#b8c8df]">{recommendation.reason}</p><dl className="mt-3 grid gap-2 border-y border-white/10 py-3 text-[9px] sm:grid-cols-3"><div><dt className="mono uppercase tracking-[0.1em] text-[#8b9085]">Asset</dt><dd className="mono m-0 mt-1 text-[#f5f4ec]">{diagnosis.assetMiB?.toFixed(2)} MiB</dd></div><div><dt className="mono uppercase tracking-[0.1em] text-[#8b9085]">Probe</dt><dd className="mono m-0 mt-1 text-[#f5f4ec]">{diagnosis.benchmarkMs?.toFixed(1)} ms</dd></div><div><dt className="mono uppercase tracking-[0.1em] text-[#8b9085]">Frame p95</dt><dd className="mono m-0 mt-1 text-[#f5f4ec]">{diagnosis.frameP95Ms?.toFixed(1)} ms</dd></div></dl><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => onSelect("auto")} className={`mono border px-2.5 py-2 text-[9px] uppercase tracking-[0.08em] ${tuningPreset === "auto" ? "border-primary bg-primary text-primary-foreground" : "border-white/20 text-[#dfe1d8] hover:border-primary"}`}>推奨を適用</button>{(["low-latency", "standard", "stable"] as const).map((preset) => <button key={preset} onClick={() => onSelect(preset)} className={`mono border px-2.5 py-2 text-[9px] uppercase tracking-[0.08em] ${tuningPreset === preset ? "border-primary bg-primary/[0.12] text-primary" : "border-white/20 text-[#a9aca2] hover:border-primary hover:text-primary"}`}>{presetLabels[preset]}</button>)}</div><p className="mono mb-0 mt-2 text-[8px] leading-4 text-[#8b9085]">設定は次回の再生開始時に反映されます。安定優先は大きい音声バッファと更新間引き、低遅延は小さいバッファを使用します。</p>{safariCompatibilityMode && <p className="mono mb-0 mt-2 text-[8px] leading-4 text-[#c2d6f4]">Safari互換モード: 長時間再生ではOPM処理を優先するため、位置・鍵盤・メーター表示を500 ms間隔に抑えます。</p>}</>}
  </section>;
}
...
export default function Home() {
  ...
  return (
    <div className="ui-dense relative min-h-screen overflow-hidden">
      <div className="noise-overlay pointer-events-none fixed inset-0 z-0 opacity-[0.035] mix-blend-screen" />
      <header className="relative z-10 border-b border-white/10 bg-[#11120f]/85 backdrop-blur-xl">
        <div className="mx-auto flex h-[64px] max-w-[1600px] items-center justify-between px-4 sm:px-6">
      ...
      <main className="relative z-10 mx-auto max-w-[1600px] px-4 py-4 sm:px-6 sm:py-5">
        <div className="grid gap-4 min-[900px]:grid-cols-[230px_minmax(0,1fr)]">
          <aside className="entry-rise flex flex-col gap-3 min-[900px]:sticky min-[900px]:top-4 min-[900px]:max-h-[calc(100vh-6rem)]">
            <section className="console-panel relative border border-white/10 bg-[#191b16]/80 p-4">
              <div className="flex items-center justify-between border-b border-white/10 pb-3"><SmallLabel>Engines</SmallLabel><Gauge size={15} className="text-[#a9aca2]" /></div>
              <div className="mt-1">
                {engineRows.map((engine) => <div className="border-b border-white/10 py-3 last:border-b-0" key={engine.id}>
                  <div className="flex items-start justify-between gap-2"><div><p className="mono m-0 text-xs font-medium text-[#f5f4ec]">{engine.label}</p><p className="mono m-0 mt-1 text-[9px] uppercase tracking-wider text-[#a9aca2]">{engine.note}</p></div><EngineStatus active={isEngineActive(engine.id)} /></div>
                  <div className="mt-3 flex items-center justify-between"><span className="mono text-[9px] uppercase tracking-[0.1em] text-[#8b9085]">Configured output level</span><span className="mono text-[10px] text-[#dfe1d8]">{activeLevels[engine.id][0]}%</span></div>
                  <div className="mt-1.5 h-1 overflow-hidden bg-white/10" role="progressbar" aria-label={`${engine.label}の設定済み出力レベル`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={activeLevels[engine.id][0]}><div className={`h-full ${engine.color}`} style={{ width: `${activeLevels[engine.id][0]}%` }} /></div>
                </div>)}
              </div>
            </section>
          ...
          <section className="entry-rise-delay flex min-w-0 flex-col">
            <div className="routing-bay panel-notch-muted faceplate order-2 relative mt-4 border border-white/15">
              <div className="flex flex-col gap-3 border-b border-white/10 p-4 sm:flex-row sm:items-end sm:justify-between sm:p-5">
                <div>
                  <SmallLabel>Source station</SmallLabel>
                  <h1 className="display mt-2 text-2xl font-semibold tracking-[-0.055em] text-[#f5f4ec] sm:text-4xl">音源をデッキへ送る。</h1>
                </div>
                <div className="flex items-center gap-2"><span className="h-2 w-2 bg-primary" /><span className="mono text-[10px] font-medium uppercase tracking-[0.17em] text-primary">Input ready</span></div>
              </div>

              <div className="grid gap-0 min-[900px]:grid-cols-[minmax(0,1fr)_270px]">
                <div className="p-4 sm:p-5">
                  <div className="flex overflow-x-auto border-b border-white/10">
                    ...
                    ] as const).map(([id, label, Icon]) => <button key={id} onClick={() => setMode(id)} className={`mono flex shrink-0 items-center gap-2 border-b-2 px-3 py-3 text-[10px] font-medium tracking-[0.12em] transition-colors ${mode === id ? "border-primary text-primary" : "border-transparent text-[#a9aca2] hover:text-[#f5f4ec]"}`}><Icon size={14} />{label}</button>)}
                  </div>
                  {recentSources.length > 0 && <div aria-label="最近使用した音源" data-testid="recent-sources" className="mt-4 border border-white/10 bg-[#11120f] p-3">
                    <div className="flex items-center justify-between gap-3"><SmallLabel>Recent sources / remote URLs</SmallLabel><button type="button" onClick={clearSourceHistory} className="mono text-[8px] uppercase tracking-[0.08em] text-[#a9aca2] hover:text-primary">Clear</button></div>
                    <p className="mono m-0 mt-1 text-[8px] leading-4 text-[#8b9085]">公開URLのみワンクリック再読込。ローカルファイルはブラウザ制約で履歴に残せません。</p>
                  ...
                  {mode === "mml" && <div className="pt-5">
                    <div className="flex items-center justify-between"><SmallLabel>MML editor / channel 01</SmallLabel><span className="mono text-[10px] text-[#a9aca2]">{mml.length} chars</span></div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="mono m-0 text-[10px] leading-5 text-[#a9aca2]">T, O, L, V, A–G, R, &lt;, &gt;, +, -, #、@OPM / @PCM / @MIDI に対応します。</p><button onClick={() => { setMml(defaultMml); setMmlError(null); }} className="mono inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.12em] text-[#d8ff3e] hover:underline"><RotateCcw size={12} />Restore sample</button></div>
                  ...
                  {mode === "local" && <div className="pt-5">
                    <div onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} onDrop={onDrop} onClick={() => sourceInputRef.current?.click()} className={`grid min-h-[190px] place-items-center border border-dashed p-5 text-center transition-colors ${isDragging ? "border-primary bg-primary/[0.07]" : "border-white/20 bg-[#11120f] hover:border-primary/60"}`}>
                      <div><div className="mx-auto grid h-12 w-12 place-items-center border border-primary/60 text-primary"><Upload size={20} /></div><p className="display mb-0 mt-5 text-xl font-semibold tracking-[-0.04em] text-[#f5f4ec]">{fileName ?? "MDR / MDX / PDXを置く"}</p><p className="mono mx-auto mt-2 max-w-sm text-[10px] leading-5 uppercase tracking-[0.08em] text-[#a9aca2]">クリックしてファイルを選択、またはここへドラッグ</p></div>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3"><p className="mono text-[10px] leading-5 text-[#a9aca2]">MDR／MDXとPDXを同時に選択、または順に追加できます。MDRは32トラックを検査し、MDXは直接、WebAssemblyのOPM／PDXコアで再生します。</p><button onClick={() => folderInputRef.current?.click()} className="mono shrink-0 border border-primary/45 bg-primary/[0.045] px-3 py-2 text-[9px] uppercase tracking-[0.08em] text-primary transition-colors hover:bg-primary hover:text-primary-foreground">Select folder / playlist</button></div>
                    {localSourceFormat === "mdx" && localMdxInfo && <div className="mt-4 border border-primary/30 bg-primary/[0.045] p-4">
                      <div className="flex items-center justify-between gap-3"><SmallLabel>MDX link analysis</SmallLabel><span className={`mono text-[9px] uppercase tracking-[0.1em] ${localPdx ? "text-primary" : "text-[#ffb8b2]"}`}>{localPdx ? "PDX READY" : localMdxPdxName ? "PDX REQUIRED" : "PDX NOT REQUIRED"}</span></div>
                      <dl className="mt-3 grid gap-x-4 gap-y-3 sm:grid-cols-2">
                      ...
                      <p className="mono mt-3 border-t border-white/10 pt-3 text-[9px] leading-5 text-[#a9aca2]">PDX候補: {localPdxCandidates.length ? localPdxCandidates.map((candidate) => candidate.name).join(" · ") : "なし"}。MDXとPDXを同時に選ぶか、必要なPDXを後から追加すると、同名ファイルを自動選択します。</p>
                    </div>}
                    ...
                    {(localMdr || loadDiagnosis.measuring) && <PlaybackAdvisorPanel diagnosis={loadDiagnosis} activeProfile={activePerformanceProfile} tuningPreset={tuningPreset} safariCompatibilityMode={safariCompatibilityMode} onSelect={setTuningPreset} />}
                  </div>}

                  {mode === "remote" && <div className="space-y-4 pt-5">
                    <div><div className="flex items-center justify-between gap-3"><SmallLabel>Remote MDR / MDX URL</SmallLabel>{isCloudShareLink(remoteMdr) && <span className="mono text-[9px] uppercase tracking-[0.09em] text-primary">Share proxy ready</span>}</div><input value={remoteMdr} onChange={(event) => setRemoteMdr(event.target.value)} placeholder="https://storage.example/song.mdr または song.mdx／Drive・Dropbox共有リンク" className="mono mt-2 w-full border border-white/15 bg-[#11120f] px-3 py-3 text-xs text-[#f5f4ec] outline-none placeholder:text-[#62675d] focus:border-primary" /></div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 border-l-2 border-primary/70 bg-primary/[0.055] p-4"><div className="flex gap-3"><AlertTriangle size={16} className="mt-0.5 shrink-0 text-primary" /><p className="m-0 text-xs leading-5 text-[#dfe1d8]">通常URLはCORS応答が必要です。Google Drive／Dropboxの<strong>公開共有リンク</strong>は、このサービスの許可済み取得経路で読み込むためブラウザ側CORSに依存しません。ログイン必須・閲覧制限・ダウンロード禁止のファイルは取得しません。</p></div><Button onClick={loadRemote} disabled={remoteLoading} className="h-9 rounded-none bg-primary px-3 text-[10px] font-semibold text-primary-foreground hover:bg-[#e5ff76]">{remoteLoading ? <LoaderCircle size={14} className="animate-spin" /> : <CloudDownload size={14} />}{remoteLoading ? "Loading" : "Load source"}</Button></div>
                    {(remoteSource || loadDiagnosis.measuring) && <PlaybackAdvisorPanel diagnosis={loadDiagnosis} activeProfile={activePerformanceProfile} tuningPreset={tuningPreset} safariCompatibilityMode={safariCompatibilityMode} onSelect={setTuningPreset} />}
                    <div className="faceplate border border-white/15 bg-[#151712] p-4">
                      <div className="flex items-center justify-between"><SmallLabel>Remote catalog / JSON</SmallLabel><span className={`mono text-[9px] ${catalogIsDriveLink ? "text-primary" : "text-[#a9aca2]"}`}>{catalogIsDriveLink ? "DRIVE DETECTED" : "CORS REQUIRED"}</span></div>
                ...
                <aside className="routing-bay-side border-t border-white/10 p-5 lg:border-l lg:border-t-0 sm:p-6">
                  ...
                  <div className="mt-5 border-t border-white/10 pt-5">
                    ...
                    <p className="mt-2 text-xs leading-5 text-[#a9aca2]">外部出力では@MIDIのノートとAll Notes Offを選択機器へ送出します。OPM・PCMはブラウザ内で鳴ります。</p>
                  </div>
                  <div className="faceplate mt-5 border-t border-white/10 pt-5">
                    <div className="flex items-center justify-between"><SmallLabel>GS SysEx / Function call</SmallLabel><span className="mono text-[9px] text-[#a9aca2]">DT1</span></div>
                    ...
                    <p className="mt-2 text-xs leading-5 text-[#a9aca2]">GS Reset、Part Rx On/OffはRoland DT1形式で送出します。テストはC4を約0.4秒鳴らし、必ずAll Notes Offで終了します。</p>
                  </div>
                  <div className="faceplate mt-5 border-t border-white/10 pt-5">
                    <div className="flex items-center justify-between"><SmallLabel>Hardware diagnostic</SmallLabel><span className={`mono text-[9px] ${midiOutputMode === "hardware" ? "text-primary" : "text-[#a9aca2]"}`}>{midiOutputMode === "hardware" ? "OUTPUT ARMED" : "SOUNDFONT MODE"}</span></div>
                    <p className="mono mt-2 text-[9px] leading-4 text-[#a9aca2]">{selectedMidiDevice ? `選択中: ${midiDevices.find((device) => device.id === selectedMidiDevice)?.name ?? "MIDI Output"}` : "外部MIDI機器を選択すると、送出バイト列を記録します。"}</p>
                    ...
                    <button onClick={exportMidiDiagnostics} disabled={!midiDiagnostics.length} className="mono mt-2 w-full border border-white/15 bg-[#11120f] px-2 py-2 text-[8px] uppercase tracking-[0.08em] text-[#dfe1d8] transition-colors hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-35">Export diagnostic JSON</button>
                  </div>
                  <div className="mt-6 border-t border-white/10 pt-5"><SmallLabel>Signal format</SmallLabel><div className="mt-3 space-y-2">{["MDR / 32 tracks", "OPM + PDX / FM & PCM", "GS MIDI / SF2 or device"].map((format) => <p key={format} className="mono m-0 flex items-center gap-2 text-[10px] text-[#d6d8d0]"><span className="h-1.5 w-1.5 bg-[#a4b99f]" />{format}</p>)}</div></div>
                </aside>
              </div>
            </div>

            <section className="primary-deck panel-notch order-1 relative mt-0 border bg-[#191b16]">
              <div className="flex flex-col gap-2 p-4 sm:gap-4 sm:p-5">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                  <div><SmallLabel>Primary instrument / program deck</SmallLabel><h2 data-testid="source-title" className="display mt-2 text-4xl font-semibold tracking-[-0.065em] text-[#f5f4ec] sm:text-5xl">{sourceTitle}</h2></div>
                  <div className="mono flex items-center gap-4 text-[11px] text-[#a9aca2]"><span>{formatTime(elapsed)}</span><span className="text-[#f5f4ec]">—</span><span>{duration ? formatTime(duration) : "--:--.---"}</span></div>
                </div>
                ...
                <div data-testid="primary-transport-controls" className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-2 border-b border-white/10 pb-3 sm:gap-3 sm:pb-4">
                  ...
                  <div className="flex min-w-0 items-center gap-1 sm:ml-auto sm:w-full sm:max-w-[420px] sm:gap-3"><Volume2 size={15} className="shrink-0 text-primary sm:h-[17px] sm:w-[17px]" /><input type="range" min="0" max="100" value={volume} onChange={(event) => { const value = Number(event.target.value); setVolume(value); audio().setMaster(value); }} className="range-control min-w-0" aria-label="マスター音量" /><span className="mono w-6 shrink-0 text-right text-[10px] text-[#dfe1d8] sm:w-8 sm:text-[11px]">{volume}</span></div>
                </div>
                <div data-testid="playback-diagnostics" className="program-diagnostics grid gap-px border border-white/10 bg-white/10 sm:grid-cols-3">
                  <div className="bg-[#11120f] px-3 py-2"><SmallLabel>Tempo / estimated</SmallLabel><p aria-label="推定BPM" className="mono m-0 mt-0.5 text-xl leading-tight text-primary">{estimatedBpm ? `${estimatedBpm.toFixed(1)} BPM` : "— BPM"}</p><p className="mono m-0 mt-0.5 text-[8px] uppercase tracking-[0.08em] text-[#8b9085]">{tempoSource}</p></div>
                  <div className="bg-[#11120f] px-3 py-2"><SmallLabel>Timer-B / OPM reg 12</SmallLabel><p aria-label="Timer-B値" className="mono m-0 mt-0.5 text-xl leading-tight text-[#f5f4ec]">{timerB === null ? "—" : `0x${timerB.toString(16).padStart(2, "0").toUpperCase()}`}</p><p className="mono m-0 mt-0.5 text-[8px] uppercase tracking-[0.08em] text-[#8b9085]">{timerBHint}</p></div>
                  <div className="bg-[#11120f] px-3 py-2"><SmallLabel>Audio clock / synced</SmallLabel><p aria-label="再生クロック" className="mono m-0 mt-0.5 text-xl leading-tight text-[#f5f4ec]">{(audioSampleRate / 1000).toFixed(1)} kHz</p><p className="mono m-0 mt-0.5 text-[8px] uppercase tracking-[0.08em] text-[#8b9085]">WASM renderer matched</p></div>
                  <div className="bg-[#11120f] px-3 py-2"><SmallLabel>MXDRV playhead</SmallLabel><p aria-label="MXDRV再生位置" className="mono m-0 mt-0.5 text-xl leading-tight text-[#f5f4ec]">{hardwarePlaybackPositionMs === null ? "—" : formatTime(hardwarePlaybackPositionMs / 1000)}</p><p className="mono m-0 mt-0.5 text-[8px] uppercase tracking-[0.08em] text-[#8b9085]">live OPM / PCM clock</p></div>
                  <div className="bg-[#11120f] px-3 py-2"><SmallLabel>GS MIDI / MXDRV delta</SmallLabel><p aria-label="GS MIDI同期差" className="mono m-0 mt-0.5 text-xl leading-tight text-[#f5f4ec]">{mdrMidiClockDeltaMs === null ? "—" : `${mdrMidiClockDeltaMs >= 0 ? "+" : ""}${mdrMidiClockDeltaMs.toFixed(0)} ms`}</p><p className="mono m-0 mt-0.5 text-[8px] uppercase tracking-[0.08em] text-[#8b9085]">event scheduled against same clock</p></div>
                </div>
                <div className="grid gap-3 border-b border-white/10 pb-5 md:grid-cols-[1fr_1fr_auto]">
                  <div className="block"><SmallLabel>Loop cap / repeats</SmallLabel><div className="mt-2 flex gap-2"><input type="number" min="1" max="99" disabled={loopCount === 0} value={loopCount === 0 ? "∞" : loopCount} onChange={(event) => setLoopCount(Math.max(1, Math.min(99, Number(event.target.value) || 1)))} aria-label="ループ回数" className="mono min-w-0 flex-1 border border-white/15 bg-[#11120f] px-3 py-2 text-xs text-[#f5f4ec] outline-none focus:border-primary disabled:cursor-not-allowed disabled:opacity-50" /><button type="button" aria-label="無限ループを切り替える" aria-pressed={loopCount === 0} onClick={() => setLoopCount((current) => current === 0 ? 1 : 0)} className={`mono border px-3 text-[10px] tracking-[0.08em] ${loopCount === 0 ? "border-primary bg-primary text-primary-foreground" : "border-white/20 text-[#dfe1d8] hover:border-primary hover:text-primary"}`}>∞</button></div><p className="mono mt-2 text-[8px] leading-4 text-[#8b9085]">∞は通常再生のみ。書き出しは安全のため1回・最大秒数に固定します。</p></div>
                  <label className="block"><SmallLabel>Export maximum / sec</SmallLabel><input type="number" min="10" max="600" value={exportLimit} onChange={(event) => setExportLimit(Math.max(10, Math.min(600, Number(event.target.value) || 60)))} className="mono mt-2 w-full border border-white/15 bg-[#11120f] px-3 py-2 text-xs text-[#f5f4ec] outline-none focus:border-primary" /></label>
                  <Button onClick={exportMml} disabled={isExporting} className="self-end rounded-none bg-[#e4e6dc] px-4 py-5 text-[10px] font-bold uppercase tracking-[0.1em] text-[#11120f] hover:bg-primary">{isExporting ? <LoaderCircle size={15} className="animate-spin" /> : <FileAudio size={15} />}{isExporting ? "Rendering" : "Export MP4"}</Button>
                </div>
                <div className="border-b border-white/10 pb-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><SmallLabel>Share session / short link</SmallLabel><p className="mt-1 text-xs leading-5 text-[#a9aca2]">公開MDR／PDX／SoundFont URL、MML、ループ上限を短い共有IDで復元します。音声・ローカルファイル・外部MIDI機器・診断ログは共有しません。既存の長い共有URLも引き続き開けます。</p></div><Button onClick={copySessionLink} disabled={sharedSessionCreate.isPending} variant="outline" className="h-10 shrink-0 rounded-none border-primary/50 px-3 text-[10px] font-medium uppercase tracking-[0.08em] text-primary hover:bg-primary/10"><Share2 size={14} />{sharedSessionCreate.isPending ? "Creating" : "Copy short link"}</Button></div>{sessionLink && <input value={sessionLink} readOnly onFocus={(event) => event.currentTarget.select()} aria-label="共有セッションリンク" className="mono mt-3 w-full border border-white/15 bg-[#11120f] px-3 py-2 text-[9px] text-[#dfe1d8] outline-none focus:border-primary" />}</div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
          <section className="console-panel relative border border-white/10 bg-[#191b16]/80 p-4 sm:p-5">
            <div className="flex items-center justify-between border-b border-white/10 pb-3"><SmallLabel>Signal routing</SmallLabel><Radio size={15} className="text-primary" /></div>
            <div className="mt-4 grid gap-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)] sm:items-center">
              <div><div className="flex items-center justify-between"><SmallLabel>Audio clock / synced</SmallLabel><span aria-label="AudioContext clock" className="mono text-xs text-primary">{(audioSampleRate / 1000).toFixed(1)} kHz</span></div><div className="signal-line mt-2 h-px opacity-70" /></div>
              <div className="grid grid-cols-3 gap-1.5">{[["OPM", "FM"], ["PCM", "PDX"], ["MIDI", "GS"]].map(([top, bottom]) => <div key={top} className="border border-white/10 bg-[#11120f] p-2 text-center"><span className="mono block text-[10px] text-primary">{top}</span><span className="mono block pt-0.5 text-[8px] tracking-widest text-[#a9aca2]">{bottom}</span></div>)}</div>
            </div>
          </section>
          <section className="border border-primary/25 bg-primary/[0.055] p-4 sm:p-5"><SmallLabel>Privacy path</SmallLabel><p className="mt-2 text-sm leading-6 text-[#e4e6dc]">音源はこのブラウザで扱います。CORS対応URLに加え、公開共有済みのGoogle Drive／Dropboxは許可済み取得経路に対応します。</p></section>
        </div>
      </main>

      <footer className="relative z-10 mx-auto mt-8 max-w-[1600px] border-t border-white/10 px-5 py-8 sm:px-8">
